...
## Data Standardization - County Names
...
## Data Standardization - Census Tract Numbers
...
The code chunk below calculates the number of current LinkUp partners per region.
```{r}
table(SNAP_retailers$region, useNA = "always") 

LinkUp_partners <- SNAP_retailers %>% filter(potential_partner == "Current Partner")
table(LinkUp_partners$region, useNA = "always")
```
